...
func (self *NodeDaoImpl) GetRebuildNode(count int64) (*Node, error) {
	...
	cur, err := collection.Find(context.Background(), bson.M{"valid": 1, "status": 1, "bandwidth": bson.M{"$lt": 50}, "timestamp": bson.M{"$gt": time.Now().Unix() - IntervalTime*2}, "version": bson.M{"$gte": 4}}, &options)
	if err != nil {
		return nil, err
	}
	result := new(Node)
