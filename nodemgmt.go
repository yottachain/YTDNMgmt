...
func NewInstance(mongoURL, eosURL, bpAccount, bpPrivkey, contractOwnerM, contractOwnerD, shadowAccount string, bpID int32, isMaster int32, config *Config) (*NodeDaoImpl, error) {
	...
	go func() {
		...
		mux.HandleFunc("/change_realspace", func(w http.ResponseWriter, r *http.Request) {
			if dao.disableBP {
				io.WriteString(w, "无BP模式下无法执行该操作")
				return
			}
			...
			realSpaceStr := r.Form.Get("realspace")
			if realSpaceStr == "" {
				w.WriteHeader(http.StatusInternalServerError)
				io.WriteString(w, "真实容量不存在！\n")
				return
			}
			realSpace, err := strconv.ParseUint(realSpaceStr, 10, 64)
			if err != nil {
				w.WriteHeader(http.StatusInternalServerError)
				io.WriteString(w, fmt.Sprintf("解析真实容量失败：%s\n", err.Error()))
				return
			}
			trxid, err := dao.eostx.ChangeRealSpace(minerid, realSpace)
			if err != nil {
				w.WriteHeader(http.StatusInternalServerError)
				io.WriteString(w, fmt.Sprintf("调用BP接口失败: %s\n", err.Error()))
				return
...
func (self *NodeDaoImpl) UpdateNodeStatus(node *Node) (*Node, error) {
	...
	if !self.disableBP && (n.ForceSync || rand.Int63n(int64(self.Config.Misc.BPSyncInterval)*100) < 100) {
		...
		// if node.ForceSync {
		// 	_, err = collection.UpdateOne(context.Background(), bson.M{"_id": node.ID}, bson.M{"$set": bson.M{"forceSync": false}})
		// 	if err != nil {
		// 		log.Printf("nodemgmt: UpdateNodeStatus: error when updating foreSync status of Node %d: %s\n", node.ID, err.Error())
		// 	} else {
		// 		node.ForceSync = false
		// 	}
		// }
	}
